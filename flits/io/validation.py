...
def validate_metadata(metadata: FilterbankMetadata) -> FilterbankMetadata:
    ...
    if not np.isfinite(metadata.tsamp) or metadata.tsamp <= 0.0:
        raise CorruptedDataError(f"Non-positive or non-finite tsamp: {metadata.tsamp!r}", path=path)
    if not np.isfinite(metadata.freqres) or metadata.freqres <= 0.0:
        raise CorruptedDataError(f"Non-positive or non-finite freqres: {metadata.freqres!r}", path=path)
    ...
    if int(metadata.header_npol) < 1:
        raise CorruptedDataError(f"header_npol must be >= 1, got {metadata.header_npol!r}", path=path)
    if int(metadata.npol) < 1:
        raise CorruptedDataError(f"npol must be >= 1, got {metadata.npol!r}", path=path)

    if not (_MJD_MIN <= metadata.start_mjd <= _MJD_MAX):
        raise CorruptedDataError(
            f"start_mjd {metadata.start_mjd!r} is outside plausible range [{_MJD_MIN}, {_MJD_MAX}]",
            path=path,
        )
    ...
    if freqs.ndim != 1 or freqs.size < 2:
        raise CorruptedDataError(
            f"freqs_mhz must be a 1-D array with at least 2 channels, got shape {freqs.shape}",
            path=path,
        )
...
__all__ = ["require_fields", "validate_metadata"]
