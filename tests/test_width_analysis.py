...
from flits.models import FilterbankMetadata, WidthAnalysisSettings
...
class WidthAnalysisTest(unittest.TestCase):
    ...
    def test_width_uncertainty_trials_can_be_disabled_for_batch_measurement(self) -> None:
        with TemporaryDirectory() as tmpdir:
            session = _synthetic_width_session(path=Path(tmpdir) / "no_width_bootstrap.fil")
            session.width_settings = WidthAnalysisSettings(
                uncertainty_trials=0,
                min_successful_trials=0,
            )

            width_analysis = session.compute_widths()

            self.assertTrue(any(result.value is not None for result in width_analysis.results))
            self.assertTrue(all(result.uncertainty is None for result in width_analysis.results))
            self.assertTrue(
                all(
                    "uncertainty_disabled" in result.quality_flags
                    for result in width_analysis.results
                    if result.value is not None
                )
            )

            session.width_settings = WidthAnalysisSettings(uncertainty_trials=-1)
